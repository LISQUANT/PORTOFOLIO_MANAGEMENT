...
class FeatureMatrix:
    def __init__(self, data_handler: DataHandler):
        ...
        self.scalers: dict[str, StandardScaler] = {}   # fitted per ticker
    ...
    def _build_feature_matrix(self, data_handler: DataHandler) -> dict[str, np.ndarray]:
        ...
        The fitted scaler is kept so NEW raw observations can be transformed
        with the same parameters at inference time (see RegimeDetector.decode_raw).
        """
        feature_matrix = {}
        ...
        for ticker in self.tickers:
            ...
            scaled, scaler = self._standardize(clean)
    ...
    def _standardize(self, matrix: np.ndarray) -> tuple[np.ndarray, StandardScaler]:
