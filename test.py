...
for name, stock_symbol in STOCKS.items():
    ...
    try:
        ...
        if not options_dates:
            atm_iv = float("nan")
            print(f"\n  No options data available for {stock_symbol} — IV skipped")
        else:
        ...
        print(f"ATM Implied Volatility    : {atm_iv:.4f}" if not np.isnan(atm_iv) else "ATM Implied Volatility    : N/A")
        print(f"IV - RV Spread            : {spread:.4f}" if not np.isnan(spread) else "IV - RV Spread            : N/A")
        ...
        print(f"IV/RV Alert : {iv_rv_signal(spread) if not np.isnan(spread) else 'N/A - no IV data'}")
